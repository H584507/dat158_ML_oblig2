...
class StreamlitApp:
    ...
    def __init__(self):
        self.model = load_model('final_model_house_values') 
    ...
    def run(self):
        ...
        st.sidebar.info('This app predicts the value of a house' )
        ...
        if add_selectbox == 'Online': 
            st.selectbox()
            mssubclass = st.selectbox('Dwelling type', ['A', 'C', 'F', '', '', '', '', ''],)
...
#             input_df = pd.DataFrame(input_dict, index=[0])
            input_dict = {'MSZoning':MSZoning}
    
            if st.button('Predict'): 
                output = self.predict(input_df)
                self.store_prediction(output)
                
                output = 'House value ' + str(output['Label'])
                #if output['Label'][0] == 1 else 'Normal'
...
get_ipython().system('jupyter nbconvert --to script 01_DAT158-streamlit_app.ipynb')
